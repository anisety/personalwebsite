...
interface ProjectsResearchTerminalItem {
  title: string;
  tech: string;
  date: string;
  details: string;
...
const Projects = ({ projects, paneLabel, projectsResearchTerminal }: ProjectsProps) => {
  const getCategoryColor = (category: string) => {
    ...
    return colors[category] || 'project-category-web';
  };
    ...
    <section id="projects" className="projects-section-new">
      <div className="about-container">
        ...
        >
          ...
          <h2 className="about-title">Projects &amp; Research</h2>
        ...
        {/* Level II order book — no links for these five entries */}
        <div className="terminal-widget terminal-widget--orderbook terminal-pane">
          <div className="terminal-widget__header">
            <span className="terminal-widget__title">LEVEL II</span>
            <span className="terminal-widget__meta">Depth + research prints</span>
          </div>
          <div className="terminal-widget__body terminal-widget__body--scroll">
            <div className="ob-grid ob-grid--head" role="row">
              <div className="ob-cell ob-cell--muted">Bid</div>
              <div className="ob-cell ob-cell--muted ob-cell--center">Spread</div>
              <div className="ob-cell ob-cell--muted">Ask</div>
            </div>

            {projectsResearchTerminal.map((p, idx) => (
              <motion.div
                key={p.title}
                ...
                role="row"
              >
                <div className="ob-side ob-side--bid">
                  <div className="ob-side__top">
                    <span className="ob-price ob-price--bid">{p.title}</span>
                    <span className="ob-meta">{p.date}</span>
                  </div>
                  <div className="ob-details">{p.details}</div>
                </div>

                <div className="ob-spread">
                  <div className="ob-spread__pill">{p.tech}</div>
                </div>

                <div className="ob-side ob-side--ask">
                  <div className="ob-side__top">
                    <span className="ob-price ob-price--ask">{p.tech}</span>
                    <span className="ob-meta">PRINT</span>
                  </div>
                  <div className="ob-details ob-details--muted">{p.title}</div>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="projects-grid-new">
          {projects.map((project, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="project-card-new terminal-pane"
            >
